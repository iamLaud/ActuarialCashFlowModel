...
class Asset(object):
    def __init__(self, return_rate=.01, value=1, time_horizon=50, starting_point=1): # we only consider Asset priced at 1$ each
        ...
        # Recuperation des raw data de l'ESG contenant les percentage changes des prix
#        self.change = pd.DataFrame(data=0, index=np.arange(1,time_horizon+1), columns=['Percentage changes'])
