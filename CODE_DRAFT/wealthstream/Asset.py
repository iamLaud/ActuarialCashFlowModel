...
class Asset(object):
    def __init__(self, return_rate=.01, value=1, time_horizon=50, starting_point=1): # we only consider Asset default value be 1$ each
        ...
        self.value = pd.DataFrame(data=value, index=np.arange(1,time_horizon+1), columns=['Market Value'])
