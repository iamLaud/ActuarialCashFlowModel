...
class Liability(object):
    def __init__(self, value=1, time_horizon=50, starting_point=1, lifespan=30, age=40):
        ...
        self.buy_back_ratio = pd.DataFrame(data=0.005, \
           index=np.arange(1,self.lifespan+1), columns=['Lapse rate']) 
        self.buy_back_ratio.iloc[3, 0] = .03 
        self.buy_back_ratio.iloc[4, 0] = .04
    ...
    def updateValue(self, current_step): # update auto des Model Points
        ...
        if(self.value.loc[current_step, 'Contract Value'] > 0 and current_step>self.starting_point):
            ...
            flagMinus = 0#-self.buyBack(current_step, percentage=0, \
                               # amount=min(basis * self.buy_back_ratio.iloc[self.contract_age, 0]\
                               # , self.value.loc[current_step-1, 'Contract Value']))
            # update deces
            
            flagMinus -= 0#self.buyBack(current_step=current_step, percentage=0, \
...
#    import gc
#    gc.enable()
#    lia = Liability(value=100, starting_point=10, lifespan=30, age=20)
...
#        lia = Liability(value=100, starting_point=10, lifespan=30, age=age)
#        for t in range(1, lia.time_horizon+1):
#            lia.update(current_step=t, mode='mid')
#            lia.update(current_step=t, mode='end')
...
#    df.axvline(13, color='black', linewidth=1.5, linestyle='--')
#    df.axvline(14, color='black', linewidth=1.5, linestyle='--')
#    df.axvline(17, color='black', linewidth=1.5, linestyle='--')
#    df.axvline(39, color='black', linewidth=1.5, linestyle='--')
#
##    df.grid(True)
#
