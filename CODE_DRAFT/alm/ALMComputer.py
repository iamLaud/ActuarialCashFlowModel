...
#        ----------- INDICATORS ----------------------------
    ...
    def balance(self, current_step, outside_stream=0):
        ...
        """
        if(current_step in range(1, self.time_horizon+1)):
            ...
            if(balance>0):
                self.assets._decrease_(amount=balance, current_step=current_step, asset_type='Bond')
                # if the balance is positive i.e. we got too many assets, we sell some
            elif(balance<0):
                self.assets._increase_(amount=abs(balance), current_step=current_step, asset_type='Bond')
...
#       --------------------------------------------------------------------
        noise = np.random.normal(0, .01, size=30) # bruit blanc de variance 1 bp sur les taux a maturite 30 ans
...
#       ------------------- DYNAMIC RAM CLEANING ---------------------
        ...
        # ---------------------------------------------------------
        simulator = ALMComputer(time_horizon=15, nb_simulation=10)
